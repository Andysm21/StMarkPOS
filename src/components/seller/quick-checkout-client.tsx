...
export function QuickCheckoutClient({ products }: { products: Product[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            <Card key={p.id}>
              <CardContent className="flex flex-col items-center gap-2 py-4 text-center">
                ...
                {qty === 0 ? (
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="icon" variant="outline" onClick={() => changeQty(p.id, -1)}>
      ...
      <div className="sticky bottom-0 -mx-4 border-t bg-card px-4 py-3 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <div>
            <p className="text-xs text-muted-foreground">{t("total")}</p>
            <p className="text-xl font-bold">{formatCurrency(total, locale)}</p>
          </div>
          <Button size="lg" className="h-12 flex-1" disabled={lines.length === 0 || submitting} onClick={confirmSale}>
            {t("confirm")}
          </Button>
        </div>
      </div>
